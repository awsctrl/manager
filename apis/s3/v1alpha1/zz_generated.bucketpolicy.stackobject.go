...
func (in *BucketPolicy) GetTemplate(client dynamic.Interface) (string, error) {
	...
	template.Description = "AWS Controller - s3.BucketPolicy (ac-{TODO})"

	template.Outputs = map[string]interface{}{
		"ResourceRef": map[string]interface{}{
			"Value": cloudformation.Ref("BucketPolicy"),
	...
	s3BucketPolicy := &s3.BucketPolicy{}

	if in.Spec.Bucket != "" {
		s3BucketPolicy.Bucket = in.Spec.Bucket
	}

	if in.Spec.PolicyDocument != "" {
		s3BucketPolicyJSON := make(map[string]interface{})
		err := json.Unmarshal([]byte(in.Spec.PolicyDocument), &s3BucketPolicyJSON)
		if err != nil {
			return "", err
		}
		s3BucketPolicy.PolicyDocument = s3BucketPolicyJSON
	}
...
func (in *BucketPolicy) GenerateStackName() string {
	return strings.Join([]string{"s3", "bucketpolicy", in.GetName(), in.GetNamespace()}, "-")
}
