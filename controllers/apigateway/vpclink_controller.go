...
// Load the apigateway VpcLink resource
// +kubebuilder:rbac:groups=apigateway.awsctrl.io,resources=vpclinks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apigateway.awsctrl.io,resources=vpclinks/status,verbs=get;update;patch

// Reconcile will make the desired state a reality
func (r *VpcLinkReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	ctx := context.Background()
	log := r.Log.WithValues("VpcLink", req.NamespacedName)

	var err error
	var instance v1alpha1.VpcLink
