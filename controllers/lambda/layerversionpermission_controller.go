...
// Load the lambda LayerVersionPermission resource
// +kubebuilder:rbac:groups=lambda.awsctrl.io,resources=layerversionpermissions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=lambda.awsctrl.io,resources=layerversionpermissions/status,verbs=get;update;patch

// Reconcile will make the desired state a reality
func (r *LayerVersionPermissionReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	ctx := context.Background()
	log := r.Log.WithValues("LayerVersionPermission", req.NamespacedName)

	var err error
	var instance v1alpha1.LayerVersionPermission
...
func (r *LayerVersionPermissionReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.LayerVersionPermission{}).
