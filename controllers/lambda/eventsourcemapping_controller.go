...
// Load the lambda EventSourceMapping resource
// +kubebuilder:rbac:groups=lambda.awsctrl.io,resources=eventsourcemappings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=lambda.awsctrl.io,resources=eventsourcemappings/status,verbs=get;update;patch

// Reconcile will make the desired state a reality
func (r *EventSourceMappingReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	ctx := context.Background()
	log := r.Log.WithValues("EventSourceMapping", req.NamespacedName)

	var err error
	var instance v1alpha1.EventSourceMapping
