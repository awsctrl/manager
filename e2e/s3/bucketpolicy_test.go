...
// RunBucketPolicySpecs allows all instance E2E tests to run
var _ = Describe("Run s3 BucketPolicy Controller", func() {

	Context("Without BucketPolicy{} existing", func() {

		It("Should create s3.BucketPolicy{}", func() {
			var stackID string
			...
			arn := bucket.Status.Outputs["Arn"]

			instance := &s3v1alpha1.BucketPolicy{
				ObjectMeta: metav1.ObjectMeta{
					GenerateName: "sample-bucketpolicy-",
					Namespace:    podnamespace,
				},
				Spec: s3v1alpha1.BucketPolicySpec{
			...
			By("Creating new s3 BucketPolicy")
			...
			Eventually(func() bool {
				By("Getting latest s3 BucketPolicy")
				instance = &s3v1alpha1.BucketPolicy{}
			...
			By("Deleting s3 BucketPolicy")
			Expect(k8sclient.Delete(context.Background(), instance)).Should(Succeed())

			By("Deleting BucketPolicy Stack")
