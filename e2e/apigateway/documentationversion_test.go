...
// RunDocumentationVersionSpecs allows all instance E2E tests to run
var _ = Describe("Run Apigateway DocumentationVersion Controller", func() {

	Context("Without DocumentationVersion{} existing", func() {

		It("Should create apigateway.DocumentationVersion{}", func() {
			var stackID string
			...
			documentationpart := &apigatewayv1alpha1.DocumentationPart{
				ObjectMeta: metav1.ObjectMeta{
			...
			Expect(k8sclient.Create(context.Background(), documentationpart)).Should(Succeed())

			instance := &apigatewayv1alpha1.DocumentationVersion{
				ObjectMeta: metav1.ObjectMeta{
					GenerateName: "sample-documentationversion-",
					Namespace:    podnamespace,
				},
				Spec: apigatewayv1alpha1.DocumentationVersionSpec{
					DocumentationVersion: "v0.0.1",
					Description:          "ApiGateway Documentation Version sample",
					RestApiRef: metav1alpha1.ObjectReference{
						ObjectRef: metav1alpha1.ObjectRef{
							Kind:       "RestApi",
							APIVersion: "apigateway.awsctrl.io/v1alpha1",
			...
			By("Creating new Apigateway DocumentationVersion")
			...
			Eventually(func() bool {
				By("Getting latest Apigateway DocumentationVersion")
				instance = &apigatewayv1alpha1.DocumentationVersion{}
			...
			By("Deleting Apigateway DocumentationVersion")
			Expect(k8sclient.Delete(context.Background(), instance)).Should(Succeed())

			By("Deleting Apigateway DocumentationPart")
			Expect(k8sclient.Delete(context.Background(), documentationpart)).Should(Succeed())
