...
// RunAccountSpecs allows all group E2E tests to run
var _ = Describe("Run IAM InstanceProfile Controller", func() {

	Context("Without InstanceProfile{} existing", func() {

		It("Should create iam.InstanceProfile{}", func() {
			var stackID string
			...
			instance := &iamv1alpha1.InstanceProfile{
				ObjectMeta: metav1.ObjectMeta{
					GenerateName: "sample-instanceprofile-",
					Namespace:    podnamespace,
				},
				Spec: iamv1alpha1.InstanceProfileSpec{
					CloudFormationMeta: metav1alpha1.CloudFormationMeta{
						Capabilities: []*string{
							&capabilityIAM,
						},
					},
					InstanceProfileName: "christopherhein",
					Path:                "/",
					Roles: []string{
						"sample-role",
					},
			...
			By("Creating new IAM InstanceProfile")
			...
			Eventually(func() bool {
				By("Getting latest IAM InstanceProfile")
				instance = &iamv1alpha1.InstanceProfile{}
			...
			By("Deleting IAM InstanceProfile")
			Expect(k8sclient.Delete(context.Background(), instance)).Should(Succeed())

			By("Deleting IAM InstanceProfile Stack")
